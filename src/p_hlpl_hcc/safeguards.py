...
1. Federated learning under DP-SGD with the manifest budget
   ``(epsilon_0, delta) = (0.4, 1e-5)`` per planned round, a design-level
   total epsilon of ``4.0`` over ``100`` rounds, gradient clip ``C = 1``,
   and noise multiplier ``sigma = 1.1``. This is a future-work envelope,
   not a privacy-accounting result from an executed federation.
...
class DPSGDConfig:
    ...
    planned_rounds: int = DP_SGD_DEFAULTS["planned_rounds"]
    epsilon_total_design_budget: float = DP_SGD_DEFAULTS[
        "epsilon_total_design_budget"
    ]
