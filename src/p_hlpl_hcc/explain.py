"""Post-hoc permutation and externally supplied attribution diagnostics."""
...
def cox_attribution_alignment(
    attributions: np.ndarray, cox_beta: np.ndarray
) -> dict[str, float]:
    """Compare an externally supplied attribution matrix with Cox ranks."""
    attribution_rank = np.argsort(np.mean(np.abs(attributions), axis=0))
    cox_rank = np.argsort(np.abs(cox_beta))
    rho, p = spearmanr(attribution_rank, cox_rank)
...
def explanation_consistency_loss(
    ...
    *,
    ...
    attribution_target = np.tanh(kappa * attributions)
    return float(np.mean(np.abs(attribution_target - cox_target)))


def clinical_rule_violation_loss(
    ...
    penalties = []
    global_attr = np.mean(attributions, axis=0)
...
def posthoc_explanation_diagnostic_report(
    proba: np.ndarray,
    y: np.ndarray,
    attributions: np.ndarray,
    cox_beta: np.ndarray,
    ...
    *,
    ...
    """Summarize post-hoc explanation diagnostics only.

    These array-based diagnostics do not enter neural optimization. The actual
    MLP Phase-E objective is implemented in ``neural.train_mlp_classifier``
    using a class-weighted focal prediction term, scalar sample-weighted
    multiclass Brier term, state-risk input gradients, and gradient-sign hinge.
    """
    ...
    exp = explanation_consistency_loss(attributions, cox_beta, x, kappa=kappa)
    clin = clinical_rule_violation_loss(attributions, feature_names)
