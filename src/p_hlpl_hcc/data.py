"""Data loading, endpoint validation, and fixture-record generation."""
...
def validate_and_prepare_dataframe(
    ...
    *,
    ...
    require_unambiguous_hard_labels: bool = True,
) -> pd.DataFrame:
    out = df.copy()
    ...
    immature_censored = (out[event_col] == 0) & (
        out[time_col] < SURVIVAL_CUTPOINTS_MONTHS[-1]
    )
    if require_unambiguous_hard_labels and immature_censored.any():
        raise ValueError(
            "A definitive eight-class endpoint is undefined for patients censored "
            "before the 72-month boundary, even when a survival_class column is "
            "supplied. Use the censoring-aware discrete-time likelihood for these "
            f"{int(immature_censored.sum())} rows."
        )
    if label_col in out.columns:
        out[label_col] = normalize_label_series(out[label_col])
    elif immature_censored.any():
        # The survival-only validation path keeps ambiguous labels missing.
        labels = pd.Series(survival_months_to_class(out[time_col].to_numpy()), index=out.index)
        labels.loc[immature_censored] = pd.NA
        out[label_col] = labels.astype("Int64")
    else:
...
def generate_fixture_hcc_records(n: int = 673, seed: int = 42) -> pd.DataFrame:
    """Create schema-complete records solely for unit and smoke software tests.

    These records are not clinical observations and must never be used for a
    paper experiment, metric reproduction, or scientific inference.
    """
    ...
    for trt in surgery:
        ...
        # Hard labels are valid only for observed events before 72 months.  The
        # C8 tail may be censored because its follow-up has crossed the horizon.
        event_prob = 0.94 - 0.05 * y
        event = 1 if y < 7 else int(rng.random() < np.clip(event_prob, 0.45, 0.95))
        ...
        row: dict[str, float | int | str] = {
            "patient_id": f"FIX{len(rows) + 1:04d}",
            "age": round(age, 3),
            "sex_male": male,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="P-HLPL-HCC data utilities")
    sub = parser.add_subparsers(dest="command", required=True)
    gen = sub.add_parser(
        "make-fixture",
        help="Create fixture-only HCC records for schema/unit/smoke software tests",
    )
    ...
    if args.command == "make-fixture":
        df = generate_fixture_hcc_records(args.n, args.seed)
        out = Path(args.out)
